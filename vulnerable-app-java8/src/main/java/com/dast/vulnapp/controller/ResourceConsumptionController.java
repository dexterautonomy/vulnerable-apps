...
public class ResourceConsumptionController {
    ...
    public Object allocate(@RequestParam int size) {
        ...
        return Compat.map("allocated", hog.length, "lastValue", hog[hog.length - 1]);
    }
    ...
    public Object spin(@RequestParam long iterations) {
        ...
        return Compat.map("iterations", iterations, "acc", acc);
    }
    ...
    public Object regexMatch(@RequestParam String pattern, @RequestParam String input) {
        try {
            ...
            return Compat.map("pattern", pattern, "matched", matched);
        } catch (Exception e) {
            return Compat.map("error", e.getMessage());
        }
    }
    ...
    public Object fib(@RequestParam int n) {
        ...
        return Compat.map("n", n, "value", fibonacci(n));
    }
    ...
    public Object report(@RequestParam(defaultValue = "1000000") int depth) {
        ...
        return Compat.map("depth", depth, "score", total);
    }
    ...
    public Object repeat(@RequestParam String text, @RequestParam int times) {
        ...
        return Compat.map("length", sb.length(), "times", times);
    }
    ...
    public Object batch(@RequestBody List<Map<String, Object>> items) {
        ...
        try {
            ...
            return Compat.map("processed", results.size());
        } catch (Exception e) {
            return Compat.map("error", e.getMessage(), "processed", results.size());
        }
    }
}
